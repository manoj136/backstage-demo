...
export const DenseTable = ({ certificates }: DenseTableProps) => {
    ...
    const data = certificates.map(certificate => {       
      return {      
        ...
        environment: ['development','production', 'sandbox', 'staging', 'test', 'uat'], //certificate.environment,
    ...
    return (
      <Table
        ...
        detailPanel={[
            {
                render: rowData => {
                    const row = rowData.rowData as unknown as Certificate;
                        ...
                        <CardContent>
                            ...
                            {filteredComponents.map((component) => (
                                <Typography color="textSecondary" gutterBottom>
                                    <Link href={targetRefEntity.replace('{namespace}',component.namespace).replace('{entity-name}', component.name)} target="_blank" rel="noopener noreferrer">
                                        {component.name}
                                    </Link> 
                                </Typography>
                            ))}   
...
export const CertificatesComponent = () => {
    ...
    const { value, loading, error } = useAsync(async (): Promise<Certificate[]> => {        
        let certificates: Certificate[] = [];
        const response = await catalogApi.getEntities({filter: {
            'spec.type': 'certificate',
            }});

            console.log('certificates: ' + JSON.stringify(response.items));
            ...
            console.log('****Certificate Infomation: ' + JSON.stringify(certificates));
            return certificates;
      }, []);
...
return(
  <Page themeId="dark">
    <Header title="Certificates Information" subtitle="">
    </Header>
    <Content>
      <ContentHeader title="">
      </ContentHeader>
